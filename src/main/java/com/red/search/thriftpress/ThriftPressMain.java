...
import com.red.search.thriftpress.RelevanceGrpcPressCall;
...
public class ThriftPressMain {
    ...
    public static void main(String[] args) throws IOException {
		...
		System.out.println("qps from config: " + qps);
    	int warmUpSeconds = 0;
    	if (conf.has("warm_up")) {
		...
		if (conf.has("duration_seconds")) {
			durationSeconds = conf.getInt("duration_seconds");
		}
...
//		SimpleRateLimiter limit = new SimpleRateLimiter(qps);
		SimpleRateLimiter limit = new SimpleRateLimiter(qps, warmUpSeconds);
		System.out.println("Main limiter=" + System.identityHashCode(limit)
				+ ", SimpleRateLimiter, total qps = " + qps
    	...
    	for (int i = 0; i < threadNum; ++i) {
    		...
    		case "dejavu_relevance":
				...
				break;
			case "relevance_grpc":
				future = service.submit(new RelevanceGrpcPressCall(limit, host, port, costThreshold, queries, serverConf, durationSeconds,warmUpSeconds));
				break;
    		default:
